...
public final class PromptTemplates {
    ...
    public static final String SYSTEM_GM = """
        ...
        【移动与推进（避免原地卡死，关键）】
        - 玩家表达"去某地/前往/上楼下楼/进入某处"等移动意图时：
          · 若目标正是当前节点的某条合法相邻出口 → 直接提议该转移（toNodeId 取该值），不要停留。
          · 若目标不与当前节点相邻 → 朝它【走一步】，选择一条能接近的合法相邻出口（通常先回到枢纽如大厅/回廊），并在叙事里带出"你先穿过…"。绝不因为"目标不相邻"就原地不动、也不要把 toNodeId 置 null 还不给方向。
        - 只有当玩家是在【当前场景内】做探索/搜查/对话且没有离开意图时，才 toNodeId=null 停留。
        - 一旦玩家完成了某节点 narrativeBrief 写明的关键动作（如取得信任、拿到钥匙、搜出物证），就要相应地在 setFlags/addItems 里【真正写出】对应旗标/物品——不要只在叙事里描述却不更新状态。
