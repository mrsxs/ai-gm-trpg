...
INSERT INTO t_scene_node (id, scenario_id, node_key, title, narrative_brief, is_ending, ending_type, sort_no) VALUES
  ...
  (3004, 1, 'node_kitchen',   '幽暗厨房',   '炉火将熄,女仆莉莉在此忙碌、神色惊惶。这是接近莉莉、累积其信任(maid_trust)的起点。温和对待会让她稍稍松口。', 0, NULL, 4),
  (3005, 1, 'node_servant',   '仆人区',     '狭窄潮湿的下人房。莉莉若已信任(maid_trust),会交出书房钥匙(set has_key)并作证看到格雷深夜出主人房(set witness_maid,这是关键证词)。未取得信任则只含糊其辞。', 0, NULL, 5),
  (3006, 1, 'node_study',     '书房密室',   '命案现场,需 has_key 进入。书桌抽屉暗格藏着埃德蒙的褪色日记(set found_diary):字里行间写他"近来日渐虚弱,疑心药石有异"。墙边书架后另有通地窖的暗道机关(可 set found_passage)。氛围:尘封、隐秘、血腥气未散。', 0, NULL, 6),
  ...
  (3009, 1, 'node_parlor',    '会客厅',     '壁炉与酒柜。律师芬奇在此整理文书。他交代背景:遗嘱近月被改、格雷被添为受益人;并可告知阁楼保险箱密码(set safe_code)。是把矛头悄悄引向格雷的关键一环。', 0, NULL, 9),
  (3010, 1, 'node_landing',   '二楼回廊',   '二楼枢纽:阴影里挂满家族肖像。可通往:主人卧室/客房/(有密码)阁楼/家族教堂,下楼回大厅。若玩家打转,可用肖像、风声渲染并点出未探之处。', 0, NULL, 10),
  (3011, 1, 'node_master',    '主人卧室',   '埃德蒙的卧室,床畔暗褐血迹、药瓶散落。惊悚场景(sanity-15)。细查可发现墙板后机关、印证书房暗道(强化 found_passage 线索),药瓶标签与格雷有关(指向 found_records)。', 0, NULL, 11),
  (3012, 1, 'node_guest',     '客房',       '格雷医生借住的客房。床头黑色药箱里藏着伪造的诊疗记录与多出的药剂(set found_records):足证他长期错误用药。格雷可能在此或闻讯赶来,谈吐镇定、回避要害。', 0, NULL, 12),
  (3013, 1, 'node_attic',     '尘封阁楼',   '需 safe_code 才能打开角落的旧保险箱。惊悚、积尘(sanity-10)。保险箱内是被篡改的遗嘱原件(set found_will):格雷的名字被添入受益人——动机铁证。', 0, NULL, 13),
  (3014, 1, 'node_cellar_entry','地窖入口', '需 found_passage(经书房暗道或主卧机关得知)才能找到并下行。湿冷石阶、铁门虚掩。通往地窖深处与暗道密室。', 0, NULL, 14),
  (3015, 1, 'node_cellar',    '阴森地窖',   '霉烂酒桶与煤堆。惊悚(sanity-10)。煤堆暗格里藏着沾血的细长手术刀——凶器(set found_weapon),以及一只丢弃的沾血手套(set found_glove)。这是最直接的物证。', 0, NULL, 15),
